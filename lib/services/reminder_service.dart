...
class ReminderService {
  ...
  Future<void> _checkAndSendOneHourReminders() async {
    try {
      ...
      _logger.info('Checking for surgeries between ${oneHourFromNow.toIso8601String()} and ${twoHoursFromNow.toIso8601String()}');
      
      // Query surgeries happening in the next 1-2 hours
      final snapshot = await _firestore
          .collection('surgeries')
          .where('status', isEqualTo: 'Scheduled')
          .where('startTime', isGreaterThanOrEqualTo: Timestamp.fromDate(oneHourFromNow))
          .where('startTime', isLessThan: Timestamp.fromDate(twoHoursFromNow))
          .get();
      
      _logger.info('Found ${snapshot.docs.length} surgeries approaching in the next hour');
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          _logger.info('Successfully sent 1-hour reminder for surgery: ${doc.id}');
        } catch (e) {
          _logger.severe('Error sending 1-hour reminder for specific surgery ${doc.id}: $e');
  ...
  Future<void> _checkAndSendTwentyFourHourReminders() async {
    try {
      ...
      _logger.info('Checking for surgeries between ${twentyFourHoursFromNow.toIso8601String()} and ${twentyFiveHoursFromNow.toIso8601String()}');
      
      // Query surgeries happening in 24-25 hours
      final snapshot = await _firestore
          .collection('surgeries')
          .where('status', isEqualTo: 'Scheduled')
          .where('startTime', isGreaterThanOrEqualTo: Timestamp.fromDate(twentyFourHoursFromNow))
          .where('startTime', isLessThan: Timestamp.fromDate(twentyFiveHoursFromNow))
          .get();
      
      _logger.info('Found ${snapshot.docs.length} surgeries approaching in 24 hours');
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          _logger.info('Successfully sent 24-hour reminder for surgery: ${doc.id}');
        } catch (e) {
          _logger.severe('Error sending 24-hour reminder for specific surgery ${doc.id}: $e');
          // Continue with next surgery even if one fails
        }
      }
    } catch (e) {
