...
class TwilioService {
  ...
  // Cloud Functions URL - replace with your actual Firebase project ID
  final String _cloudFunctionUrl = 'https://us-central1-orscheduler-9f1d0.cloudfunctions.net/sendSMS';
  ...
  TwilioService() : 
    ...
    _logger.info('TwilioService initialized with: ${_accountSid != null ? 'SID: YES' : 'SID: NO'}, ${_authToken != null ? 'Token: YES' : 'Token: NO'}, ${_twilioNumber != null ? 'Number: $_twilioNumber' : 'Number: NO'}');
  }
  ...
  Future<bool> sendSMS({
    ...
    try {
      ...
      String formattedNumber = toNumber.toString().replaceAll(RegExp(r'[^\+\d]'), '');
  ...
  Future<bool> _sendDirectSMS(String toNumber, String messageBody) async {
    try {
      ...
      final twilioEndpoint = 'https://api.twilio.com/2010-04-01/Accounts/$_accountSid/Messages.json';
      ...
      _logger.info('Twilio API response: Status ${response.statusCode}, Body length: ${response.body.length}');
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        try {
          ...
          final errorMessage = responseData['message'] as String? ?? 'No message provided';
          final errorCode = responseData['code'].toString(); // Convert to string in case it's a number
