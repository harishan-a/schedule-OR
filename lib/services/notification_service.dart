...
class NotificationService {
  ...
  Future<bool> sendScheduledNotification(String surgeryId) async {
    try {
      ...
      if (!surgeryDoc.exists) {
        _logger.warning('Cannot send scheduled notification: Surgery not found');
        return false;
      }
      
      final surgeryData = surgeryDoc.data()!;
      final String surgeryType = surgeryData['surgeryType'] ?? 'Surgery';
      final DateTime startTime = (surgeryData['startTime'] as Timestamp).toDate();
      ...
      final List<String> phoneNumbers = await _getPersonnelPhoneNumbers(surgeryData);
      
      // Message template
      final String message = 'Your $surgeryType is scheduled for $formattedDate at $formattedTime. Please check your app for details.';
  ...
  Future<bool> sendApproachingNotification(
    ...
    try {
      final surgeryDoc = await _firestore.collection('surgeries').doc(surgeryId).get();
      if (!surgeryDoc.exists) {
        _logger.warning('Cannot send approaching notification: Surgery not found');
        return false;
      }
      ...
      // Message template
      final String message = 'Reminder: You have a $surgeryType scheduled in $hoursBeforeSurgery $hourText (on $formattedDate at $formattedTime). Please prepare accordingly.';
  ...
  Future<bool> sendUpdateNotification(
    ...
    try {
      if (!newData.containsKey('surgeryType') || !newData.containsKey('startTime')) {
        _logger.warning('Cannot send update notification: Missing required fields');
        return false;
      }
      
      final String surgeryType = newData['surgeryType'] ?? 'Surgery';
      final DateTime oldStartTime = (oldData['startTime'] as Timestamp).toDate();
      final DateTime newStartTime = (newData['startTime'] as Timestamp).toDate();
      ...
      // Get phone numbers for all involved personnel
      final List<String> phoneNumbers = await _getPersonnelPhoneNumbers(newData);
      
      // Message template
      String message = 'Notice: Your $surgeryType scheduled for $oldFormattedDate at $oldFormattedTime has been updated.';
  ...
  Future<bool> sendStatusChangeNotification(
    ...
    try {
      final surgeryDoc = await _firestore.collection('surgeries').doc(surgeryId).get();
      if (!surgeryDoc.exists) {
        _logger.warning('Cannot send status change notification: Surgery not found');
        return false;
      }
      ...
      // Message template
      final String message = 'Update: The status of your $surgeryType at $formattedDate, $formattedTime is now "$newStatus". Check your app for details.';
  ...
  Future<List<String>> _getPersonnelPhoneNumbers(Map<String, dynamic> surgeryData) async {
  ...
  Future<String?> _getUserPhoneNumber(String userName) async {
    try {
      final querySnapshot = await _firestore
          .collection('users')
          .where('firstName', isEqualTo: userName.split(' ').first)
          .get();
  ...
  Future<bool> _sendSMSIfEnabled({
    ...
    try {
      // Log for debugging
      _logger.info('Attempting to send SMS to $phoneNumber for $notificationType');
