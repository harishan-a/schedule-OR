...
class NotificationManager {
  static final NotificationManager _instance =
      NotificationManager._internal(false);
  static final NotificationManager _webInstance =
      NotificationManager._internal(true);
  ...
  final StreamController<Map<String, dynamic>> _notificationsStreamController =
    ...
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized) {
        ...
        if (token != null) {
          ...
          _logger.warning(
              'No messaging token available, continuing without push capability');
        }
        ...
        _logger.warning(
            'FCM permissions not granted: ${settings.authorizationStatus}');
      }
  ...
  Future<String?> _getIOSToken() async {
    ...
    try {
      ...
      if (apnsToken != null) {
        ...
        _logger.warning(
            'No APNS token available, push notifications may not work');
    ...
    try {
      ...
      if (Platform.isIOS) {
        try {
          ...
          if (apnsToken == null) {
            _logger.info(
                'Skip subscribing to topic $topic: APNS token not available, but continuing app operation');
          ...
          if (e.toString().contains('apns-token-not-set')) {
            _logger.info(
                'APNS token not available for topic subscription, continuing without push notifications');
  ...
  Future<void> unsubscribeFromTopic(String topic) async {
    try {
      ...
      if (Platform.isIOS) {
        ...
        if (apnsToken == null) {
          _logger.warning(
              'Cannot unsubscribe from topic $topic: APNS token not available');
          return;
        }
      }
  ...
  /// Store a notification in Firestore for a user
  Future<void> storeNotification({
    ...
    try {
      ...
      if (user != null) {
        ...
        // Stream to app UI immediately with a client-side timestamp for immediate display
        _notificationsStreamController.add({
          ...
          'timestampCreated': Timestamp.now().millisecondsSinceEpoch,
          'source': 'direct',
        });
      }
    } catch (e) {
  ...
  Future<void> sendScheduledNotification({
    ...
    try {
      ...
      final String body =
          'A ${surgery.surgeryType} surgery has been scheduled for ${_formatDateTime(surgery.startTime)}';
      ...
      try {
        await _firestore.collection('surgery_notifications').add({
          'userId': userId,
          'title': title,
          'body': body,
          'data': data,
        ...
        _logger.info(
            'Stored scheduled notification in central collection for user $userId');
      } catch (e) {
      ...
      if (userPrefs['enablePushNotifications'] == true) {
        try {
          await showLocalNotification(
              title: title, body: body, payload: userId);
      ...
      try {
        if (userPrefs['enableSmsNotifications'] == true) {
          ...
          if (userData != null && userData['phoneNumber'] != null) {
            ...
            if (formattedNumber.contains('1234') ||
                formattedNumber.length < 10) {
              _logger.warning(
                  'Skipping obvious test phone number: $formattedNumber');
              return;
            }
  ...
  Future<void> sendScheduledNotificationById(String surgeryId) async {
    try {
      ...
      _logger
          .info('Personnel names for scheduled notification: $personnelNames');
      ...
      for (final name in personnelNames) {
        ...
        if (userId != null) {
          _logger.info(
              'Found user ID $userId for name $name, sending scheduled notification');
          ...
          _logger.warning(
              'Could not find user ID for name: $name, skipping scheduled notification');
        }
      }
    } catch (e) {
  ...
  Future<void> sendApproachingNotification({
    ...
    try {
      ...
      final String body =
          'Reminder: ${surgery.surgeryType} surgery is in $timeRemaining';
      final Map<String, dynamic> data = {
        'type': 'approaching',
        'surgeryId': surgery.id,
      };
      ...
      try {
        ...
        _logger.info(
            'Stored approaching notification in surgery_notifications collection for user $userId');
      } catch (e) {
      ...
      try {
        ...
        if (userData != null && userData['phoneNumber'] != null) {
          final phoneNumber = userData['phoneNumber'];
          _logger.info(
              'Sending SMS reminder to $phoneNumber for approaching surgery');
        ...
        _logger.severe(
            'Error sending SMS for approaching notification: $smsError');
      }
    } catch (e) {
  ...
  Future<void> sendApproachingNotificationById(String surgeryId,
      {int hoursBeforeSurgery = 1}) async {
    try {
      _logger.info(
          'Sending approaching notification for surgery $surgeryId ($hoursBeforeSurgery hour(s) before)');
      ...
      final String timeRemaining =
          '$hoursBeforeSurgery hour${hoursBeforeSurgery > 1 ? 's' : ''}';
      ...
      try {
        await _firestore.collection('surgery_notifications').add({
          ...
          'involvedPersonnel': [
            ...
            if (surgeryData['technologists'] != null)
              ...surgeryData['technologists'],
          ],
        });

        _logger.info(
            'Stored approaching reminder in central surgery_notifications collection');
      } catch (e) {
  ...
  Future<void> sendStatusChangeNotification({
    ...
    try {
      _logger.info(
          'Sending status change notification to user $userId for surgery ${surgery.id} ($oldStatus → $newStatus)');

      // Get user preferences - but we'll force SMS to be enabled for status changes
      final userPrefs = await _getUserPreferences(userId);
      if (userPrefs == null) {
        _logger.warning(
            'No preferences found for user $userId, skipping notification');
        return;
      }
      ...
      final String body =
          'The ${surgery.surgeryType} surgery has changed from $oldStatus to $newStatus';
      final Map<String, dynamic> data = {
        'type': 'status',
        'surgeryId': surgery.id,
        'oldStatus': oldStatus,
      ...
      try {
        ...
        _logger
            .info('Stored notification in user\'s collection for user $userId');
      } catch (e) {
      ...
      try {
        ...
        if (userData != null && userData['phoneNumber'] != null) {
          final phoneNumber = userData['phoneNumber'];
          _logger.info(
              'Forcing SMS to $phoneNumber for user $userId regardless of preferences');
      ...
      _logger
          .info('Successfully sent all enabled notifications to user $userId');
    } catch (e) {
  ...
  Future<void> sendStatusChangeNotificationById(
      String surgeryId, String oldStatus, String newStatus) async {
    try {
      _logger.info(
          'Sending status change notification for surgery $surgeryId ($oldStatus → $newStatus)');
      ...
      if (currentUser == null) {
        _logger.warning(
            'No authenticated user found for status change notification');
        return;
      }
      ...
      try {
        ...
        _logger.info(
            'Stored status change in central surgery_notifications collection');
      } catch (e) {
      ...
      for (final userId in notifyUserIds) {
        ...
        if (userId == currentUser.uid && notifyUserIds.length > 1) {
          _logger.info(
              'Skipping duplicate notification to current user who made the change');
          continue;
        }
  ...
  Future<void> sendUpdateNotification({
    ...
    try {
      ...
      final String body =
          'The ${surgery.surgeryType} surgery scheduled for ${_formatDateTime(surgery.startTime)} has been updated';
      final Map<String, dynamic> data = {
        'type': 'update',
        'surgeryId': surgery.id,
      };
      ...
      _logger.info(
          'Stored update notification in user\'s collection for user $userId');
      ...
      try {
        ...
        if (userData != null && userData['phoneNumber'] != null) {
          final phoneNumber = userData['phoneNumber'];
          _logger.info(
              'Forcing SMS to $phoneNumber for user $userId for update notification');
  ...
  Future<void> sendUpdateNotificationById(String surgeryId,
    ...
    try {
      ...
      // Create surgery object using the factory constructor
      final surgery = Surgery.fromFirestore(
          surgeryId, surgeryDoc.data() as Map<String, dynamic>);
      ...
      if (newData['surgeon'] != null &&
          newData['surgeon'] != oldData['surgeon']) {
      ...
      personnelNames
          .addAll([...oldNurses, ...newNurses, ...oldTechs, ...newTechs]);

      _logger.info('Personnel names for update notification: $personnelNames');

      // Get the current user who's making the change (to ensure we have at least one recipient)
      final currentUser = _auth.currentUser;
      if (currentUser != null) {
        personnelNames.add(currentUser.displayName ?? 'Current User');
      }

      // Check if we have a detailed change message
      String notificationBody;
      if (newData.containsKey('changeMessage') &&
          newData['changeMessage'] is String) {
        // Use the detailed change message
        notificationBody = newData['changeMessage'];
        _logger.info('Using detailed change message: $notificationBody');
      } else {
        // Create a generic message about changes
        ...
        final String oldFormattedDateTime =
            DateFormat('MMM d, y h:mm a').format(oldStartTime);
        final String newFormattedDateTime =
            DateFormat('MMM d, y h:mm a').format(newStartTime);

        // Base message about the surgery being updated
        notificationBody =
            'The $surgeryType scheduled for $oldFormattedDateTime has been updated.';

        // Add info about time changes if applicable
        if (oldStartTime != newStartTime) {
          notificationBody += ' New time: $newFormattedDateTime.';
        }

        // Add info about location changes if applicable
        final oldRoom = oldData['room'] is List
            ? (oldData['room'] as List).join(', ')
            : oldData['room']?.toString() ?? 'unknown room';

        final newRoom = newData['room'] is List
            ? (newData['room'] as List).join(', ')
            : newData['room']?.toString() ?? 'unknown room';

        if (oldRoom != newRoom) {
          notificationBody += ' Room changed to $newRoom.';
        }

        // Add info about staff changes
        final oldSurgeon = oldData['surgeon']?.toString() ?? '';
        final newSurgeon = newData['surgeon']?.toString() ?? '';
        if (oldSurgeon != newSurgeon && newSurgeon.isNotEmpty) {
          notificationBody += ' Surgeon changed to $newSurgeon.';
        }
      }

      // FALLBACK APPROACH: First store notification in the surgery_notifications collection
      // This is less restrictive according to the security rules
      try {
        await _firestore.collection('surgery_notifications').add({
          'surgeryId': surgeryId,
          'timestamp': FieldValue.serverTimestamp(),
          ...
          'changeMessage': newData['changeMessage'],
          'involvedPersonnel': personnelNames.toList(),
        });

        _logger
            .info('Stored update in central surgery_notifications collection');
      } catch (e) {
        _logger.severe('Error storing central update notification: $e');
        // Continue execution despite this error - don't return
      }

      // Now handle user-specific notifications with proper error handling
      for (final name in personnelNames) {
        try {
          // Find the actual user ID for this name
          final userId = await _findUserIdByName(name);
          ...
          if (userId != null) {
            _logger.info(
                'Found user ID $userId for name $name, sending update notification');
            ...
            // Try to store notification in user's collection
            try {
              await _firestore
                  .collection('users')
                  .doc(userId)
              ...
              _logger.warning(
                  'Failed to store notification for user $userId: $notifError');
              // Continue with other notifications
            }

            // Send local notification if app is running
            try {
              if (userPrefs['enablePushNotifications'] == true) {
                await showLocalNotification(
                    title: title, body: body, payload: userId);
              ...
              _logger.warning('Failed to send local notification: $pushError');
            }
            ...
            try {
              ...
              _logger.warning('Failed to send email notification: $emailError');
            }
            ...
            try {
              ...
              if (userData != null && userData['phoneNumber'] != null) {
                final phoneNumber = userData['phoneNumber'];
                _logger.info(
                    'Sending SMS to $phoneNumber for user $userId for update notification');
                ...
                _logger
                    .info('Sending SMS to formatted number: $formattedNumber');
                ...
                _logger
                    .warning('No phone number found for user $userId for SMS');
              }
            } catch (smsError) {
              _logger.severe(
                  'Error sending SMS notification for update: $smsError');
            }
          } else {
            _logger.warning(
                'Could not find user ID for name: $name, skipping update notification');
          }
        } catch (userError) {
          _logger.warning(
              'Error processing notification for user ${name}: ${userError}');
        }
      }

      // Also notify current user separately if they aren't in personnel list
      try {
        if (currentUser != null) {
          final currentUserName = currentUser.displayName;
          final bool isAlreadyNotified = personnelNames
              .any((name) => name == currentUserName || name == 'Current User');

          if (!isAlreadyNotified) {
            _logger.info('Also notifying current user: ${currentUser.uid}');

            // Direct approach to store notification for current user
            try {
              await _firestore
                  .collection('users')
                  .doc(currentUser.uid)
              ...
              _logger.info(
                  'Directly stored notification for current user ${currentUser.uid}');

              // Also try to show local notification
              try {
                await showLocalNotification(
                    title: 'Surgery Updated',
                    body: notificationBody,
                ...
                _logger
                    .warning('Failed to show local notification: $pushError');
              }
            } catch (storeError) {
              _logger.warning(
                  'Failed to store notification for current user: $storeError');
            }
          }
        }
      } catch (e) {
        _logger.warning('Error sending notification to current user: $e');
      }
    } catch (e) {
  ...
  Future<void> sendPushNotification({
    ...
    try {
      ...
      if (tokensDocs.docs.isEmpty) {
        _logger.info(
            'No FCM tokens found for user $userId, sending local notification');
  ...
  Future<bool> _sendEmail(
    ...
    try {
      if (_emailApiKey == null ||
          _emailSendEndpoint == null ||
  ...
  Future<List<UserNotificationData>> _getInvolvedUsers(
    ...
    try {
      ...
      return CombineLatestStream.combine2(
          ...
          (List<Map<String, dynamic>> direct,
              List<Map<String, dynamic>> surgery) {
  ...
  Stream<List<Map<String, dynamic>>> _getUserDirectNotificationsStream(
      String userId) {
    return _firestore
        .collection('users')
        .doc(userId)
      ...
      return snapshot.docs.map((doc) {
        ...
        return {
          ...
          'timestampCreated': Timestamp.now()
              .millisecondsSinceEpoch, // Add unique timestamp for widget keys
          'source': 'direct',
        };
      }).toList();
    });
  ...
  Stream<List<Map<String, dynamic>>> _getUserSurgeryNotificationsStream(
      User user) {
    ...
    return _firestore
      ...
      return _firestore
        ...
        for (final doc in snapshot.docs) {
          final data = doc.data();
          final List<String> involvedPersonnel =
              List<String>.from(data['involvedPersonnel'] ?? []);
  ...
  Future<void> markNotificationAsRead(String notificationId,
    ...
    try {
      ...
      _logger.info(
          'Marked notification $notificationId as read (source: $source)');
    } catch (e) {
  ...
  Future<void> clearNotification(String notificationId,
      [String source = 'direct']) async {
    try {
      final user = _auth.currentUser;
      if (user == null) return;
      ...
      if (source == 'direct') {
        ...
        // For surgery notifications, we'll delete them from Firestore
        // rather than just marking them as read, to ensure persistence
        await _firestore
            .collection('surgery_notifications')
            .doc(notificationId)
            .delete();
      ...
      // Rethrow to allow UI to handle it
      rethrow;
    }
  }
  ...
  Future<void> clearAllNotifications() async {
    try {
      ...
      // Also delete all relevant surgery notifications
      await _clearSurgeryNotifications(user);

      _logger.info('Cleared all notifications for user ${user.uid}');
    } catch (e) {
  ...
  /// Delete surgery notifications for a user
  Future<void> _clearSurgeryNotifications(User user) async {
    try {
      // First get user details to match against involved personnel
      final userDoc = await _firestore.collection('users').doc(user.uid).get();
      if (!userDoc.exists) return;
      ...
      // Get notifications this user is involved in
      final snapshot =
          await _firestore.collection('surgery_notifications').get();
      ...
      if (count > 0) {
        await batch.commit();
        _logger.info('Deleted $count surgery notifications');
      }
    } catch (e) {
      _logger.severe('Error clearing surgery notifications: $e');
      rethrow;
    }
  }
  ...
  Stream<int> _getSurgeryUnreadNotificationCount(User user) {
    ...
    return _getUserSurgeryNotificationsStream(user).map((notifications) =>
        notifications
    ...
    try {
      ...
      return {
        ...
        'enableEmailNotifications':
            userData['enableEmailNotifications'] ?? false,
      };
    } catch (e) {
  ...
  Future<bool> sendTestSMS(
      ...
      String message = 'Test notification from ORScheduler'}) async {
    try {
  ...
  Future<String?> _findUserIdByName(String name) async {
    try {
      ...
      if (nameParts.length >= 2) {
        ...
        if (query.docs.isNotEmpty) {
          _logger.info(
              'Found user with firstName/lastName: ${query.docs.first.id}');
          return query.docs.first.id;
        }
      }

      // Special handling for role-prefixed names (like "Nurse John Smith")
      if (name.startsWith("Nurse ") ||
          name.startsWith("Doctor ") ||
          name.startsWith("Technologist ") ||
          name.startsWith("Dr. ")) {
        String actualName;
        if (name.startsWith("Nurse ")) {
          actualName = name.substring("Nurse ".length);
        } else if (name.startsWith("Doctor ")) {
          actualName = name.substring("Doctor ".length);
        } else if (name.startsWith("Technologist ")) {
          actualName = name.substring("Technologist ".length);
        } else {
          // Dr.
          actualName = name.substring("Dr. ".length);
        }

        _logger.info('Trying lookup without role prefix: $actualName');
        ...
        if (query.docs.isNotEmpty) {
          _logger.info(
              'Found user with fullName (no role): ${query.docs.first.id}');
          return query.docs.first.id;
        }
        ...
        if (actualNameParts.length >= 2) {
          ...
          if (query.docs.isNotEmpty) {
            _logger.info(
                'Found user with firstName/lastName (no role): ${query.docs.first.id}');
            return query.docs.first.id;
          }
        }

        // Last resort - just try by first name after removing role
        query = await _firestore
            .collection('users')
        ...
        if (query.docs.isNotEmpty) {
          _logger.info(
              'Found user with just firstName (no role): ${query.docs.first.id}');
          return query.docs.first.id;
        }
      }
  ...
  Future<void> _sendFCMNotification(
    ...
    try {
      ...
      _logger
          .info('FCM message details: title=$title, body=$body, token=$token');
  ...
  Future<void> sendScheduledNotificationByUserId({
    ...
    try {
      _logger.info(
          'Sending scheduled notification for surgery $surgeryId to user $userId');
      ...
      _logger.info(
          'Successfully sent scheduled notification to user $userId for surgery $surgeryId');
    } catch (e) {
      _logger.severe(
          'Error sending scheduled notification to user $userId for surgery $surgeryId: $e');
    }
  }
}
