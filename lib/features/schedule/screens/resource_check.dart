  ...
  Future<void> _checkConflicts() async {
    ...
    try {
      ...
      if (_operatingRoom != null) {
        final roomConflicts =
            await query.where('room', isEqualTo: _operatingRoom).get();
      ...
      if (_selectedDoctor != null) {
        final doctorConflicts =
            await query.where('surgeon', isEqualTo: _selectedDoctor).get();
      ...
      for (var nurse in _selectedNurses) {
        final nurseConflicts =
            await query.where('nurses', arrayContains: nurse).get();
        ...
        child: ListView(
          children: [
            ...
            ListTile(
              ...
              subtitle:
                  Text(DateFormat('MMM dd, yyyy - hh:mm a').format(_startTime)),
              onTap: () async {
                  ...
                  if (pickedTime != null) {
                    setState(() {
                      ...
                      if (_endTime.isBefore(_startTime)) {
                        _startTime =
                            _endTime.subtract(const Duration(hours: 1));
                      }
                    });
                  }
                }
